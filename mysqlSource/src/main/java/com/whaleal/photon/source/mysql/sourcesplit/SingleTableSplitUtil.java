...
import com.whaleal.photon.common.util.split.RangeSplitWrap;
...
//        JdbcTemplate jdbcTemplate = MySqlConnection.getJdbcTemplate(com.whaleal.photon.core.programInfo.getSourceDsName());
...
//        }
        ...
        if (null != rangeList && !rangeList.isEmpty()) {
            ...
            //pluginParams.add(com.whaleal.photon.core.programInfo); // this is wrong for new & old split
            tempQuerySql = buildQuerySql(column, table, where)
                    + (hasWhere ? " and " : " where ")
...
//                }
    ...
    private static String genPkRangeSQL(ProgramInfo programInfo, String table, String where) {
        String splitPkName = null;
//        boolean hasSplitPk = StringUtils.isNotBlank(com.whaleal.photon.core.programInfo.getSplitPk());
//        splitPkName = hasSplitPk ? com.whaleal.photon.core.programInfo.getSplitPk().trim() : SingleTableSplitUtil.getPK(table, com.whaleal.photon.core.programInfo);
        if (StringUtils.isNotBlank(programInfo.getSplitPk())) {
            splitPkName = programInfo.getSplitPk();
        } else {
            splitPkName = getResultPK(programInfo, table);
...
//        String where = com.whaleal.photon.core.programInfo.getString(Key.WHERE, null);
    ...
    /**
     * a wrapped method to com.whaleal.photon.target.mongodb.execute select-like sql statement .
     *
